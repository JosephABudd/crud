...
pub const View = struct {
    ...
    pub fn frame(
...

        {
            ...
            dvui.labelNoFmt(@src(), "Select A Contact To Remove", .{ .font_style = .title }) catch |err| {
    ...
    pub fn setState(self: *View, settings: Options) !void {
        ...
        self.state.?.reset(self.allocator, settings) catch |err| {
            self.exit(@src(), err, "Remove.Select unable to set state");
            return err;
        };
    ...
    fn handleClick(self: *View, contact_list_record: *const ContactListRecord) !void {
        try self.all_panels.Remove.?.view.?.setState(.{ .contact_list_record = contact_list_record });
        self.all_panels.setCurrentToRemove();
    }

    /// The screen calls hasList to determine if this screen will frame.

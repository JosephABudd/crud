...
pub const View = struct {
    ...
    pub fn frame(
        ...
        }
        {
            ...
            if (try dvui.button(@src(), "Cancel", .{}, .{})) {
                ...
                // Switch to the select panel if there are contacts to select from.
                if (self.all_panels.Select.?.view.?.hasList()) {
    ...
    pub fn setState(self: *View, settings: Options) !void {
        ...
        self.state.?.reset(self.allocator, settings) catch |err| {
            self.exit(@src(), err, "Contacts.Add unable to set state");
            return err;
        };

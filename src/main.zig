...
pub fn main() !void {
    ...
    // The store.
    const app_data_path: []const u8 = try std.fs.getAppDataDir(gpa, "crud");
    defer gpa.free(app_data_path);
    std.fs.makeDirAbsolute(app_data_path) catch |err| {
        if (err != error.PathAlreadyExists) {
            return;
        }
    };
    var params = [2][]const u8{ app_data_path, "store.sqlite" };
    const store_path: [:0]const u8 = try std.fs.path.joinZ(gpa, &params);
    var store: *Store = try Store.init(gpa, store_path);
    defer store.deinit();
